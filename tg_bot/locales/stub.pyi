...
https://www.youtube.com"""]: ...
    ...
    @staticmethod
    def less_than_1_links() -> Literal["""❌ Количество ссылок не может быть меньше 1"""]: ...
...
💰 Сумма: &lt;code&gt;{ $crypto_amount }&lt;/code&gt;"""]: ...
    ...
    @staticmethod
    def when_selected(*, package, price, balance) -> Literal["""Вы выбрали пакет:
...
Продолжить 🤔?"""]: ...
    ...
    @staticmethod
    def buyed_packeg(*, package) -> Literal["""Готово! Вы выбрали пакет: { $package }
...
Ожидайте, выполнение заказа может занять до 12 дней"""]: ...
    ...
    @staticmethod
    def ref(*, count_referrals, count_money, referral_link) -> Literal["""Привет! 👋 У нас уже работает система рефералов, которая позволяет тебе получать бонусы за привлечение новых пользователей к нашему сервису.
...
Твоя реферальная ссылка: { $referral_link }"""]: ...
    ...
    @staticmethod
    def notif_usr(*, referral_reward) -> Literal["""Вы получили реферальное вознаграждение: { $referral_reward }$. 💰 Благодарим за поддержку нашего сервиса! 👍"""]: ...
